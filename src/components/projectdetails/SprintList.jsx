...
import { Calendar, Users, Edit, Play, CheckCircle, ChevronDown, ChevronUp, ArrowRight } from "lucide-react";
...
// Normalize possible shapes: array or { items: [...] }
function toArray(x) {
...
export default function SprintList({
  ...
  // Normalize once
  const safeSprints = toArray(sprints);
  const safeStories = toArray(stories);

  const getSprintProgress = (sprint) => {
    const sprintStories = safeStories.filter((s) => s?.sprint_id === sprint?.id);
    const completedStories = sprintStories.filter((s) => s?.status === "done");
    return sprintStories.length
  ...
  const getSprintStats = (sprint) => {
    ...
    const totalPoints = sprintStories.reduce(
      (sum, s) => sum + (Number(s?.story_points) || 0),
      0
    );
  ...
  const handleStatusChange = async (sprint, newStatus) => {
    try {
      ...
      const { updateProjectStatus } = await import(
        "@/components/utils/projectStatusUpdater"
      );
  ...
  const getInitials = (email) => {
    ...
    const nameParts = email
      ...
      .split(/\s+/)
      .filter(Boolean);
    return nameParts.map((n) => n[0]).slice(0, 2).join("").toUpperCase() || "U";
  };

  if (!safeSprints.length) {
    return (
    ...
    <Card className="shadow-lg border-0">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Sprint List ({safeSprints.length})
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {safeSprints.map((sprint) => {
            ...
            const daysRemaining = sprint?.end_date
              ...
              : 0;
            const sprintStories = safeStories.filter(
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <span>
                      Progress ({stats.completedPoints}/{stats.totalPoints} pts)
                    </span>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                  <Link
                    to={createPageUrl(
                      `SprintBoard?project_id=${sprint?.project_id}&sprint=${sprint?.id}`
                    )}
                  >
                    ...
                    >
                      <div className="space-y-2">
                        {sprintStories.map((story) => (
                          <div
                            key={story?.id}
                            className="flex items-center justify-between p-3 rounded-md bg-slate-50/70 border border-slate-100"
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <Badge
                                  variant="secondary"
                                  className={`${
                                    storyStatusColors[story?.status] ||
                                ...
                                <Badge variant="outline" className="text-xs">
                                  {Number(story?.story_points) || 0} pts
                                </Badge>
                              </div>
                            </div>
                            {story?.assignee && (
